...
export const checkIn = async (req, res) => {
  try {
    ...
    toDate.setHours(23, 59, 59, 999); // Set to end of day for expiry check
    console.log('Today (start of day):', today);
    console.log('Valid From (start of day):', fromDate);
    console.log('Valid To (end of day):', toDate);
    // LOGIC: Pass valid if:
    // 1. validFrom <= today
    // 2. today <= validTo (end of day)
    if (fromDate > today) {
      console.log('❌ Pass not yet valid');
      return res.status(400).json({ 
        message: `Pass not yet valid. Valid from: ${fromDate.toLocaleDateString()}`
      });
    }
    if (today > toDate) {
      console.log('❌ Pass has expired');
      return res.status(400).json({ 
        message: `Pass expired. Valid until: ${toDate.toLocaleDateString()}`
      });
    }
